...
module sigma_fold_core #(
    ...
    generate
        for (scratch_bank = 0; scratch_bank < FOLD_K;
            ...
            wire b_wr = i_B_wr_en && !o_busy && B_wr_addr_valid &&
                        (B_wr_bank_lane == scratch_bank);
            wire [A_AW-1:0] a_wr_addr = A_wr_bank_addr[A_AW-1:0];
            wire [B_AW-1:0] b_wr_addr = B_wr_bank_addr[B_AW-1:0];
            ...
            sigma_sync_ram #(
                ...
                .clk(clk), .wr_en(b_wr),
                .wr_addr(b_wr_addr), .wr_data(i_B_wr_data),
    ...
    reg [OUT_DATA_TYPE-1:0] fold_add_a;
    reg [OUT_DATA_TYPE-1:0] fold_add_b;
    ...
    generate
        for (result_bank = 0; result_bank < MAX_N;
            ...
            wire stream_c_accum = (PIPELINED_FOLD_ACCUM != 0) &&
                                  (state == CORE_ACCUM_STREAM) &&
                                  accum_pipe_valid[ACCUM_PIPE_LATENCY-1] &&
                                  (accum_write_col == result_bank);
            ...
            wire [OUT_DATA_TYPE-1:0] c_wr_data = c_accum ? accumulated_value :
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            for (accum_pipe_index = 0;
                 ...
                 accum_pipe_index = accum_pipe_index + 1)
            ...
            case (state)
                ...
                CORE_ACCUM_STREAM: begin
                    ...
                    if (accum_read_pending) begin
                        fold_add_a <= C_bank_q[
                            accum_pending_col*OUT_DATA_TYPE +: OUT_DATA_TYPE];
                        fold_add_b <= fold_bank_q[
                            accum_pending_col*OUT_DATA_TYPE +: OUT_DATA_TYPE];
                    ...
                    if (accum_issue_addr != accum_total) begin
                        ...
                        accum_pending_last <=
                            (accum_issue_addr + 1'b1 >= accum_total);
                        accum_read_pending <= 1'b1;
                        accum_issue_addr <= accum_issue_addr + 1'b1;
                        if (accum_issue_col_index + 1'b1 >= r_N) begin
                            accum_issue_col_index <= 0;
                            accum_issue_row_index <=
                                accum_issue_row_index + 1'b1;
                        end else begin
                            accum_issue_col_index <=
                                accum_issue_col_index + 1'b1;
                        end
                    end else begin
