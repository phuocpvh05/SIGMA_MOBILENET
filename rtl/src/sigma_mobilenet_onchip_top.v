...
module sigma_mobilenet_onchip_top #(
    ...
    // The generated descriptor also carries the later packed-ROM offset.
    // MOB7 intentionally ignores it and uses the board-proven linear ROM,
    // but retaining this sink keeps one descriptor artifact compatible with
    // both layouts; synthesis removes the unused register.
    ...
    reg b_read_valid_d1, b_read_valid_d2;
    reg [15:0] b_core_addr_d1, b_core_addr_d2;
    ...
    reg core_start, core_A_wr_en, core_B_wr_en;
    ...
    sigma_fold_core #(
        ...
        .PIPELINED_FOLD_ACCUM(1),
        // Eight physical banks would otherwise consume more fragmented BRAM
        // than the 245k-word model leaves available on the ZU-5EV.  The tile
        // scratchpads are small and map efficiently into LUTRAM.
        .SCRATCH_RAM_STYLE("distributed"),
...
`undef SIGMA_MOBILENET_LAYER_CASE
                ...
                S_LAYER_READY: begin
                    if ((layer_M == 0) || (layer_N == 0) || (layer_K == 0) ||
                        ...
                        if ((layer_kind == KIND_DEPTHWISE) &&
                            ...
                            fast_mode_m1 <= 0;
                            state <= S_DW_START;
                        end else if ((layer_kind == KIND_CONV) &&
                                     (layer_M == 1) &&
                                     (layer_K <= 256) && !layer_residual) begin
                            fast_mode_m1 <= 1;
                            state <= S_DW_START;
                        end else
                            state <= S_PREP_TILE;
                ...
                S_DW_WAIT: begin
                    if (dw_done) begin
                        if (dw_error) begin
                            ...
                            // No current model ends in depthwise, but retaining
                            // this guard keeps the controller descriptor-safe.
                            state <= S_DONE;
                        end else begin
                ...
                S_B_INDEX: begin
                    if (layer_kind == KIND_DEPTHWISE)
                        ...
                        b_weight_product <= b_global_k * layer_N;
                    state <= S_B_ADDR;
                end
                ...
                S_B_STREAM: begin
                    // The ROM and banked B scratchpad accept one BF16 word per
                    // cycle after priming.  Address induction replaces the
                    // former five-state loop around every individual weight.
                    b_read_valid_d2 <= b_read_valid_d1;
                    b_core_addr_d2 <= b_core_addr_d1;
                    b_read_valid_d1 <= 1'b0;
                    if (b_issue_count < chunk_K) begin
                        weight_rd_en <= 1'b1;
                        weight_rd_addr <= b_stream_weight_addr;
                        b_read_valid_d1 <= 1'b1;
                        b_core_addr_d1 <=
                            b_col * MAX_TOTAL_K + b_issue_count;
                        ...
                        if (layer_kind == KIND_DEPTHWISE)
                            ...
                            b_stream_weight_addr <=
                                b_stream_weight_addr + layer_N;
                    end
                    if (b_read_valid_d2) begin
                        core_B_wr_en <= 1'b1;
                        core_B_wr_addr <= b_core_addr_d2;
                        core_B_wr_data <= weight_rd_data;
                    end
                    if ((b_issue_count >= chunk_K) &&
                ...
                S_CORE_WAIT: begin
                    if (core_error) begin
                        ...
                        if ((k_base == 0) && final_chunk && !layer_residual)
                ...
                S_C_STREAM: begin
                    ...
                    if (core_C_rd_valid) begin
                        ...
                        if (layer_idx == LAST_LAYER) begin
                            if (!argmax_valid ||
                                (float_key(processed_value) > float_key(argmax_value))) begin
                                argmax_valid <= 1'b1;
                                argmax_value <= processed_value;
                                argmax_index <= n_base + c_resp_col;
                            end
                        end
                        ...
                        if (c_resp_count + 1'b1 >= c_total_count) begin
                            if ((layer_M > MAX_M) &&
                                ...
                                if (!argmax_valid ||
                                    (float_key(processed_value) > float_key(argmax_value)))
                                    o_prediction <= n_base + c_resp_col;
                                else
