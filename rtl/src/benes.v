...
module benes # (
	...
	parameter NUM_PES = 2, // fixed project DPE width
	parameter LEVELS = 3) ( // 2*(log2PE) + 1
	clk,
	rst,
	...
	wire [NUM_PES * DATA_TYPE -1 : 0] r_data_bus_ff = i_data_bus;
	wire [2*(LEVELS-2)*NUM_PES + NUM_PES-1 : 0] r_mux_bus_ff = i_mux_bus;
	...
	// The Flex-DPE already pipelines data/control and tracks validity. Reset
	// muxes on every Benes bit only consume LUTs and are not architecturally
	// observable while the valid stream is low.
