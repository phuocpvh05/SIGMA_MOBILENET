...
`ifndef POST_SYNTH_NETLIST
    localparam WEIGHT_PHYSICAL_ROWS = 16158;
    reg [63:0] golden_bank0 [0:WEIGHT_PHYSICAL_ROWS-1];
    reg [63:0] golden_bank1 [0:WEIGHT_PHYSICAL_ROWS-1];
    reg [63:0] golden_bank2 [0:WEIGHT_PHYSICAL_ROWS-1];
    reg [63:0] golden_bank3 [0:WEIGHT_PHYSICAL_ROWS-1];
    reg wide_v1 = 0, wide_v2 = 0, wide_v3 = 0, wide_v4 = 0;
    reg [17:0] wide_a1 = 0, wide_a2 = 0, wide_a3 = 0, wide_a4 = 0;
    reg super_v1 = 0, super_v2 = 0, super_v3 = 0, super_v4 = 0;
    reg [17:0] super_a1 = 0, super_a2 = 0, super_a3 = 0, super_a4 = 0;

    function automatic [63:0] expected_wide(input [17:0] address);
        begin
            case (address[1:0])
                0: expected_wide = golden_bank0[address >> 2];
                1: expected_wide = golden_bank1[address >> 2];
                2: expected_wide = golden_bank2[address >> 2];
                default: expected_wide = golden_bank3[address >> 2];
            endcase
        end
    endfunction

    function automatic [255:0] expected_super(input [17:0] address);
        begin
            expected_super = {
                golden_bank3[address >> 2], golden_bank2[address >> 2],
                golden_bank1[address >> 2], golden_bank0[address >> 2]
            };
        end
    endfunction
`endif
...
`ifndef POST_SYNTH_NETLIST
    // Score every ROM response at the exact clock where a consumer captures
    // it.  This localizes a bad final prediction to the first misaligned
    // weight transaction instead of merely reporting the final class.
    always @(posedge clk) begin
        if (!resetn) begin
            wide_v1 <= 0; wide_v2 <= 0; wide_v3 <= 0; wide_v4 <= 0;
            super_v1 <= 0; super_v2 <= 0; super_v3 <= 0; super_v4 <= 0;
        end else begin
            wide_v4 <= wide_v3;
            wide_a4 <= wide_a3;
            wide_v3 <= wide_v2;
            ...
            wide_v2 <= wide_v1;
            wide_a2 <= wide_a1;
            wide_v1 <= dut.u_accelerator.store_weight_wide_rd_en;
            wide_a1 <= dut.u_accelerator.store_weight_wide_rd_addr;
            ...
            super_v2 <= super_v1;
            super_a2 <= super_a1;
            super_v1 <= dut.u_accelerator.dw_busy &&
                        dut.u_accelerator.dw_weight_super_rd_en;
            super_a1 <= dut.u_accelerator.dw_weight_super_rd_addr;

            if (dut.u_accelerator.b_read_valid_d5 ||
                dut.u_accelerator.u_depthwise4.weight_valid_d5) begin
                if (!wide_v4) begin
                    $display("MOBILENET_WEIGHT_SCOREBOARD FAILED: wide consumer without response");
                    $finish;
                end
                if (dut.u_accelerator.weight_wide_rd_data !==
                    expected_wide(wide_a4)) begin
                    $display("MOBILENET_WEIGHT_SCOREBOARD FAILED: wide addr=%0d got=%016h expected=%016h layer=%0d",
                        wide_a4, dut.u_accelerator.weight_wide_rd_data,
                        expected_wide(wide_a4),
                        dut.u_accelerator.layer_idx);
                    $finish;
                end
            end

            if (dut.u_accelerator.u_depthwise4.m1_weight_valid_d5) begin
                if (!super_v4) begin
                    $display("MOBILENET_WEIGHT_SCOREBOARD FAILED: super consumer without response");
                    $finish;
                end
                if (dut.u_accelerator.weight_super_rd_data !==
                    expected_super(super_a4)) begin
                    $display("MOBILENET_WEIGHT_SCOREBOARD FAILED: super addr=%0d got=%064h expected=%064h layer=%0d",
                        super_a4, dut.u_accelerator.weight_super_rd_data,
                        expected_super(super_a4),
                        dut.u_accelerator.layer_idx);
                    $finish;
                end
            end
...
`endif
    ...
    task automatic run_image(input integer which, input [3:0] expected);
        ...
        begin
            ...
            value = 0;
            polls = 0;
            while (!value[1] && polls < 10000) begin
            ...
            if ((value < 540000) || (value > 560000)) begin
    ...
    initial begin
`ifndef POST_SYNTH_NETLIST
        $readmemh("mobilenet_onchip_bf16_bank0.mem", golden_bank0);
        $readmemh("mobilenet_onchip_bf16_bank1.mem", golden_bank1);
        $readmemh("mobilenet_onchip_bf16_bank2.mem", golden_bank2);
        $readmemh("mobilenet_onchip_bf16_bank3.mem", golden_bank3);
`endif
