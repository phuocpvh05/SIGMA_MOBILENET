# Refresh the existing PS-AXI MobileNet project after the MOB6 weight ROM was
# timing-hardened.  This script updates generated BD
...
if {[get_property NAME [current_project]] ne "sigma_mobilenet_ps"} {
    error "Wrong project. Open rtl/vivado/build/sigma_mobilenet_ps.xpr first"
}
catch {close_design}
...
foreach bank {0 1 2 3} {
    ...
    if {![file exists $path]} {
        error "Missing MobileNet BRAM image: $path"
    }
    lappend bank_files $path
    if {[llength [get_files -quiet $path]] == 0} {
        add_files -fileset sources_1 -norecurse $path
        puts "Added [file tail $path]"
    }
}
...
# This release must be synthesized from the current MOB6 RTL.  An old project
...
# pre-MOB6 netlist.  Vivado 2025.1 exposes this property as numeric 1/0.
...
puts "============================================================"
puts "SIGMA MobileNet BRAM ROM update complete"
puts "ROM        : 4 x 64-bit BRAM banks, 16158 rows/bank"
puts "Cascade    : maximum 4 RAMB36 blocks per hard cascade"
puts "Bandwidth  : 256 bits/clock (unchanged)"
puts "Latency    : 4 clocks (input register + three-cycle XPM read)"
puts "Scratchpad : A/B fold banks use BRAM; small result banks remain LUTRAM"
puts "Profile    : MOB6 with pipelined ROM and replicable bank-local address registers"
puts "Guardband  : 0.200 ns setup + 0.050 ns hold at the 300 MHz board clock"
puts "Simulator  : Verilog-only"
puts "Incremental: disabled; synth_1 must rebuild MOB6 from RTL"
