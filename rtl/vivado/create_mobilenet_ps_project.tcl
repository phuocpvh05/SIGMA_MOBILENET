...
set vmod_dir [file join $project_dir project_1.srcs sources_1 imports vmod]
...
# export an unnecessary functional VHDL netlist; the initialized model ROM
# expands that file and can keep Vivado busy long after the
...
add_files -fileset sources_1 -norecurse [file join $cnn_dir mobilenet_onchip_bf16.mem]
...
# path is synchronous at the realizable 250 MHz PS clock.  This keeps all AXI
# frequency metadata consistent and adds physical timing margin.
set_property -dict [list \
    ...
    CONFIG.PSU__CRL_APB__PL0_REF_CTRL__FREQMHZ {250}] $ps
...
if {[info exists SIGMA_GUI_ONLY] && $SIGMA_GUI_ONLY} {
    ...
    puts "PL clock   : 250 MHz"
    puts "Guardband  : 0.200 ns setup + 0.050 ns hold required at 250 MHz"
